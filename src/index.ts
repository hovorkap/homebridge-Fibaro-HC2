...
import {platformName, pluginName, ShadowFactory} from './shadows'
...
class FibaroHC2 {
	...
	constructor(log: (format: string, message: any) => void, config: Config, api: any) {
		...
		this.config = config;
		this.shadowFactory = new ShadowFactory(Accessory, Service, Characteristic, this);
	...
	LoadAccessories(devices) {
		...
		this.log('Gathered base types:', `${JSON.stringify(this.shadowFactory.baseTypes.filter(onlyUnique))}`);

		// Create Security System accessory
		if (this.config.securitysystem == "enabled") {
			let device = {name: "FibaroSecuritySystem", roomID: 0, id: 0};
			let sa = this.shadowFactory.createShadowSecuritySystemAccessory(device);
		...
		if (this.config.switchglobalvariables && this.config.switchglobalvariables != "") {
			...
			for (let i = 0; i < globalVariables.length; i++) {
				...
				let sa = this.shadowFactory.createShadowGlobalVariableSwitchAccessory(device);
				this.addAccessory(sa);
			}
		}
	...
	bindCharacteristicEvents(characteristic, service) {
		...
		service.isHarmonyDevice = !!(IDs.length >= 4 && IDs[4] == "HP");
		service.isLockSwitch = !!(IDs.length >= 4 && IDs[4] == "LOCK");
